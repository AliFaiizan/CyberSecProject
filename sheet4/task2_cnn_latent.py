...
# ============================================================
# PER-FOLD CNN (NO scenario_fn inside)
# ============================================================
def run_cnn_latent_per_fold(
    Z_train, y_train,
    Z_test,  y_test,
    fold_idx, out_dir,
    attack_id=None,
    ...
    """
    Train/eval CNN on a single (pre-split) fold.
    Saves Predictions_Fold{fold_idx+1}.csv in out_dir.

    Returns:
      (fold_idx, attack_id, model, y_pred, y_test_w, total_runtime, total_memory, detail_dict)
    """

    os.makedirs(out_dir, exist_ok=True)

    print(f"\n[CNN] Fold {fold_idx+1}" + (f"   Attack={attack_id}" if attack_id is not None else ""))
    ...
    if len(X_train_w) == 0 or len(X_test_w) == 0:
        print("[CNN] Not enough windows — skipping fold.")
        detail = {"window_time": t_window, "window_mem": mem_window}
        return (fold_idx, attack_id, None, None, None, 0.0, 0, detail)
    ...
    # 4) Build + Train CNN
    # -------------------------------
    model = build_cnn((M, Z_train.shape[1]), dropout_rate=dropout_rate, lr=lr)
    ...
    # Save predictions (window-based)
    pd.DataFrame({"predicted_label": y_pred, "Attack": y_test_w}).to_csv(
        f"{out_dir}/Predictions_Fold{fold_idx+1}.csv", index=False
    )
    ...
    return (fold_idx, attack_id, model, y_pred, y_test_w, total_runtime, total_memory, detail)