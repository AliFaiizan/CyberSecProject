#!/usr/bin/env python3
import os
import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
from matplotlib_venn import venn3
from upsetplot import UpSet


# ---------------------------------------------------------
# Load error indices for one fold
# ---------------------------------------------------------
def load_errors(csv_path):
    df = pd.read_csv(csv_path)
    df.columns = [c.strip().replace("\ufeff", "") for c in df.columns]
    y_true = df.iloc[:, 0].values
    y_pred = df.iloc[:, 1].values
    return set(np.where(y_pred != y_true)[0])


# ---------------------------------------------------------
# Find prediction files
# ---------------------------------------------------------
def find_prediction_files(folder):
    return sorted(
        os.path.join(folder, f)
        for f in os.listdir(folder)
        if f.endswith(".csv") and "Predictions_Fold" in f and "metrics" not in f.lower()
    )


# ---------------------------------------------------------
# Venn diagram (Scenario 1)
# ---------------------------------------------------------
def plot_venn3(err1, err2, err3, labels, out):
    plt.figure(figsize=(7, 7))
    venn3([err1, err2, err3], set_labels=labels)
    plt.title("Classification Error Overlap")
    plt.tight_layout()
    plt.savefig(out)
    plt.close()
    print("[SAVED]", out)


# ---------------------------------------------------------
# UpSet plot (Scenario 2 & 3)
# ---------------------------------------------------------
def plot_upset(error_dict, fold_idx, out):
    models = list(error_dict.keys())

    rows = []
    for model, indices in error_dict.items():
        for idx in indices:
            rows.append({"id": idx, "model": model})

    df = pd.DataFrame(rows)
    if df.empty:
        print("[WARN] No errors to plot")
        return

    df = (
        df.assign(value=True)
          .pivot_table(index="id", columns="model", values="value", fill_value=False)
    )

    for m in models:
        if m not in df.columns:
            df[m] = False

    df = df[models]
    df.index = pd.MultiIndex.from_frame(df)
    df = df.iloc[:, 0:0]

    plt.figure(figsize=(14, 10))
    upset = UpSet(
        df,
        subset_size="count",
        show_counts=True,
        sort_by="cardinality",
        with_lines=True,
        intersection_plot_elements=12,
        element_size=40
    )
    upset.plot()

    plt.suptitle(f"Classification Error Overlap – Fold {fold_idx}", fontsize=18, y=1.02)
    plt.savefig(out, bbox_inches="tight", dpi=200)
    plt.close()
    print("[SAVED]", out)


# ---------------------------------------------------------
# Main
# ---------------------------------------------------------
def run_task3b(scenario):

    base_dir = f"exports/Scenario{scenario}"
    out_dir = f"task3b_errors/Scenario{scenario}"
    os.makedirs(out_dir, exist_ok=True)

    print(f"[INFO] Processing Scenario {scenario}")

    if scenario == 1:
        model_paths = {
            "OCSVM": f"{base_dir}/OCSVM",
            "LOF": f"{base_dir}/LOF",
            "EE": f"{base_dir}/EllipticEnvelope",
        }
    else:
        model_paths = {
            "SVM": f"{base_dir}/SVM",
            "kNN": f"{base_dir}/kNN",
            "RF": f"{base_dir}/RandomForest",
            "CNN": f"{base_dir}/CNN",
        }

    folds = {m: find_prediction_files(p) for m, p in model_paths.items()}
    num_folds = len(next(iter(folds.values())))

    for f in range(num_folds):
        print(f"\n[INFO] FOLD {f+1}")

        errors = {}
        for model in model_paths:
            pred_file = folds[model][f]
            print(f"[LOADING] {model} → {pred_file}")
            errors[model] = load_errors(pred_file)

        if scenario == 1:
            out = f"{out_dir}/venn_scenario1_fold{f+1}.png"
            plot_venn3(
                errors["OCSVM"],
                errors["LOF"],
                errors["EE"],
                labels=("OCSVM", "LOF", "EllipticEnvelope"),
                out=out,
            )
        else:
            out = f"{out_dir}/upset_scenario{scenario}_fold{f+1}.png"
            plot_upset(errors, f+1, out)


# ---------------------------------------------------------
# CLI
# ---------------------------------------------------------
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Task 3(b): Error Overlap")
    parser.add_argument("--scenario", type=int, required=True, choices=[1, 2, 3])
    args = parser.parse_args()
    run_task3b(args.scenario)
