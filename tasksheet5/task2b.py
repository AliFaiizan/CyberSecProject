...
from utils import load_data, create_windows_for_vae, load_feature_ranking
...
def main():
    ...
    X_df.columns = [f"z{i}" for i in range(X_df.shape[1])]
    ...
    # Train ML models
    # ---------------------------------------------------------
    importance_dir = os.path.join("feature_importance", f"Scenario{sc}")
    for name, fn in model_map.items():
        # run_and_save(name, fn, X_df, y_series, scenario_fn, k, sc, out_base)

        imp_csv = os.path.join(importance_dir, f"{name}_aggregated_importance.csv")

        if not os.path.exists(imp_csv):
            print(f"[SKIP] No aggregated importance file for {name}: {imp_csv}")
            continue

        ranked_features = load_feature_ranking(imp_csv)

        # Keep only features that exist in X_df (safety)
        ranked_features = [f for f in ranked_features if f in X_df.columns]
        print(ranked_features)
        if len(ranked_features) == 0:
            print(f"[SKIP] Ranked feature list is empty after filtering for {name}")
            continue

        # Run in batches of 4: 4, 8, 12, ...
        for topk in range(4, len(ranked_features) + 1, 4):
            selected = ranked_features[:topk]
            X_sub = X_df[selected]

            exp_name = f"{name}_{topk}"
            print(f"\nRunning {exp_name} with {topk} features: {selected}")
            run_and_save(exp_name, fn, X_sub, y_series, scenario_fn, k, sc, out_base)
